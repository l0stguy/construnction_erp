        ...
        <div class="page-wrapper">
            ...
            <div class="row page-titles">
                <div class="col-md-5 align-self-center">
                    <h3 class="text-primary">Utilization Management</h3> </div>
                <div class="col-md-7 align-self-center">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
                        <li class="breadcrumb-item active">Utilization Management</li>
                    </ol>
                </div>
            </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="input-states">
                                    <form class="form-horizontal" method="POST" action="php_action/order.php" id="createOrderForm">
                                     <div class="form">
<div class="form-group">
                                            <div class="row">
                                              ...
                                              <label class="col-sm-2 control-label">Select Project</label>
                                               <div class="col-sm-4">
                                                <select class="form-control" name="projectName" id="projectName<?php echo $x; ?>" onChange="Dept5(this);" >
                    <option value="">~~SELECT~~</option>
                    <?php
                      $productSql = "SELECT * FROM project WHERE delete_status = 0";
                    ...
                    ?>
                                                ...
                                                <div class="col-sm-4">
                                                 <input type="text" class="form-control address" id="address" placeholder="Client Address" name="address" autocomplete="off" readonly="" />
                                               </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <div class="row">
                                              ...
                                              <label class="col-sm-2 control-label">Utilization No</label>
                                               <div class="col-sm-4">
                                                <?php $user = "select count(*) as cnt from order";
                                                 $result = $connect->query($user);
                                                $new=10000+$result['cnt']+1;
                                                ?>
                                                <input type="text" class="form-control" placeholder="Utilization No" value="<?php echo sprintf('%04d',intval($new)) ?>" name="uno" autocomplete="off" required/>
                                               </div>
<?php $current_date=date('Y-m-d'); ?>

                                                <label class="col-sm-2 control-label">Utilization Date</label>
                                                <div class="col-sm-4">
                                                 <input type="date" class="form-control" id="orderDate" value="<?php echo$current_date;?>" name="orderDate" autocomplete="off" />
                                               </div>
                                            </div>
                                        </div>
                                     <div class="form-group">
                                            <div class="row">
                                                ...
                                                <div class="col-sm-4">
                                                 <input type="text" class="form-control   clientName" id="clientName" name="clientName" placeholder="Client Name" autocomplete="off" readonly="" />
                                               </div>

                                                <label class="col-sm-2 control-label">Client Contact No.</label>
                                                <div class="col-sm-4">
                                                <input type="text" class="form-control clientno" id="clientno" name="clientContact" placeholder="Contact Number No." autocomplete="off" pattern="^[0][1-9]\d{9}$|^[1-9]\d{9}$" readonly required/>
                                               </div>
...
//$('.product').change(function(){
    var val=$(eve).val();
     var current=$(eve).closest('.form');
    //$(current).find('.group_id').val(10);
    //console.log(rate);
    //alert(current);
 $.ajax({ 
...
url: "php_action/cruddemo2.php",

data: "dept_id="+val,
dataType:'JSON',
// 
success: function(response){
    //alert(response['display1'][0].clientname);
    
//alert($(current).find('.clientName').val(response['display1'][0].clientname));
    
    
    $(current).find('.address').val(response['display1'][0].address);
    $(current).find('.clientName').val(response['display1'][0].clientname);
    $(current).find('.clientno').val(response['display1'][0].clientno);
    // $(current).find('.comp').val(response['display1'][0].company1);
    // $(current).find('.addrs').val(response['display1'][0].address);
    // $(current).find('.city').val(response['display1'][0].city);
    // $(current).find('.country').val(response['display1'][0].country);
    // $(current).find('.cno').val(response['display1'][0].cno);
    // $(current).find('.email').val(response['display1'][0].email);
    // $(current).find('.idtype').val(response['display1'][0].idtype);
    // $(current).find('.ino').val(response['display1'][0].ino);
